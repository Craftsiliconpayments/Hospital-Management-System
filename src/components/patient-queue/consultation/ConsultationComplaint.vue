      ...
      <div class="page-content">
        <div class="container-fluid">
                      ...
                      <div class="row">
                        <div class="col-12 mb-2">
                          <div class="bg-warning border-2 bg-soft p-3 mb-4 d-flex align-items-center">
                            <i class="mdi mdi mdi-comment-outline font-28 mr-15px"></i>
                            <p class="fw-semibold mb-0 pb-0 text-uppercase">Record the patient's main complaint</p>
                          </div>
                        </div>
                            ...
                            <div class="row">
                              <div class="col-12">
                                <table class="table table-editable-2 align-middle table-edits">
                                  ...
                                  <tbody>
                                  <tr v-for="(complaint, index) in complaints" :key="index" :data-id="complaint.id">
                                    <td style="width: 80px">{{ index + 1 }}.</td>
                                    <td>
                                      <input class="form-control" type="text" v-model="complaint.complain" placeholder="Enter Complaint e.g. Fever">
                                    </td>
                                    <td class="text-right cell-change d-flex align-items-center justify-content-end">
                                      <a class="btn btn-light btn-rounded waves-effect btn-circle btn-transparent" @click="removeComplaint(index)" title="Remove"><i class="bx bx-x"></i></a>
                                    </td>
                                  </tr>
                                  </tbody>
                                  <tfoot>
                                  <tr>
                                    <td colspan="4" class="bg-light add-field-2 cursor-pointer" @click="addComplaint">
          <span class="d-flex align-items-center text-uppercase">
            ...
            <span class="pl-5">Add Complaint</span>
          </span>
                                    </td>
                                  </tr>
                                  </tfoot>
                              ...
                              <div class="col-lg-6 col-md-6 col-sm-12">
                                <div class="mb-4">
                                  ...
                                  <textarea v-model="patientNotes" name="patientNotes" id="patient-notes" class="form-control" placeholder="Enter relevant notes about the patient's condition, treatment, or observations here" rows="4" spellcheck="false" data-ms-editor="true"></textarea>
                                </div>
                              </div>
                        ...
                        <div class="col-12">
                          <div class="d-flex justify-content-end">
                            <button @click="addMainComplain()" class="btn btn-primary waves-effect waves-light text-uppercase mr-3 me-2">
                      ...
                      <div class="row">
                        <div class="col-12 mb-2">
                          <div class="bg-info border-2 bg-soft p-3 mb-4 d-flex align-items-center">
                            <i class="mdi mdi-history font-28 mr-15px"></i>
                            <p class="fw-semibold mb-0 pb-0 text-uppercase">Previous Complains</p>
                          </div>
                        </div>
                        ...
                        <div class="col-12">
                          <div class="table-responsive">
                            <table class="table table-striped">
                              ...
                              <tbody>
                              <tr v-for="(item, index) in mainComplains" :value="item" :key="index">
                                <td @click="selectMainCompain(item)"> <a href="javascript: void(0);">{{formatDate(item.dateCreated)}}</a></td>
                                <td>{{item.complain}}</td>
                                <td>{{item.patientNotes}}</td>
                                <td>{{item.doctor}}</td>
                              </tr>
                              </tbody>
                            </table>
...
export default {
  ...
  data() {
    return {
      ...
      complaints: [
        { id: 1, complain: "",patientNotes:"" }, // Initial empty complaint row
      ],
      patientNotes:'',
      mainComplains: [
     {
  ...
  methods: {
    ...
    // Method to add a new complaint row
    addComplaint() {
      const newId = this.complaints.length + 1;
      this.complaints.push({ id: newId, text: "" });
    },
    // Method to remove a complaint row by index
    ...
    addMainComplain() {
      this.loading = true
      const data = new FormData();
      data.append("function", "addMainComplain");
      data.append("complaints", JSON.stringify(this.complaints));
      data.append("idNo", sessionStorage.getItem("idNo"));
      data.append("doctor", sessionStorage.getItem("sessionNames"));
      data.append("patientNotes", this.patientNotes);
    ...
    selectMainCompain(item){
      this.complaints.splice(0)
      this.complaints.push(item)
      this.patientNotes = this.complaints[0].patientNotes
    },
    getMainComplains() {
      const data = new FormData();
      data.append("function", "getMainComplains");
      data.append("visitCode", sessionStorage.getItem("visitCode"));
      data.append("idNo", sessionStorage.getItem("idNo"));
      execute(data, patient)
          .then((res) => {
            this.loading = false
            this.mainComplains.splice(0)
            if (res.data.success) {
              this.complaints.splice(0);

              if (res.data && res.data.data && res.data.data.complaints) {
                this.complaints = res.data.data.complaints
                this.patientNotes = this.complaints[0].patientNotes
              }

              if (res.data && res.data.data && res.data.data.mainComplains) {
                this.mainComplains = res.data.data.mainComplains
              }
