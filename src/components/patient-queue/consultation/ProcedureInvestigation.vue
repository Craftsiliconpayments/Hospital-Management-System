        ...
        <div class="container-fluid">
          ...
          <div class="row">
                      ...
                      <div class="row">
                        <div class="col-12 mb-2">
                          <div class="bg-warning border-2 bg-soft p-3 mb-4 d-flex align-items-center">
                            ...
                            <p class="fw-semibold mb-0 pb-0 text-uppercase">Order a lab test or radiology exam for the patient</p>
                          </div>
                        </div>
                        <div class="col-12">
                              ...
                              <div class="col-12">
                                ...
                                <table class="table table-editable-2 align-middle table-edits">
                                  ...
                                  <tr v-for="(item, index) in invoiceItem" :key="index">
                                    ...
                                    <td class="text-right">
                                      <a @click="removeInvoiceItem(index)" class="btn btn-light btn-rounded waves-effect btn-circle btn-transparent" title="Cancel">
                                        <i class="bx bx-x"></i>
                                      </a>
                                    </td>
                                  </tr>
                                  ...
                                  <tfoot>
                                  <tr>
                                    <td colspan="4" class="bg-light add-field-2 cursor-pointer" @click="addInvoiceItem">
            <span class="d-flex align-items-center text-uppercase">
              ...
              <span class="pl-5">Add Diagnosis</span>
            </span>
                                    </td>
                                  </tr>
                                  </tfoot>
                        ...
                        <div class="col-12">
                          <div class="d-flex justify-content-end">
                            ...
                            <button @click="addToBill()" class="btn btn-primary waves-effect waves-light text-uppercase mr-3 me-2">
                      ...
                      <div class="row">
                        <div class="col-12 mb-2">
                          <div class="bg-info border-2 bg-soft p-3 mb-4 d-flex align-items-center">
                            ...
                            <p class="fw-semibold mb-0 pb-0 text-uppercase">Previous Lab Test</p>
                          </div>
                        </div>
                        ...
                        <div class="col-12">
                          <div class="table-responsive">
                            <table class="table table-striped">
                              ...
                              <tbody>
                              <tr v-for="(item, index) in examinations" :value="item" :key="index">
                                <td @click="selectItem(item)"> <a href="javascript: void(0);">{{formatDate(item.dateCreated)}}</a></td>
                                <td>{{item.type}}</td>
                                <td>{{item.exam}}</td>
                                <td>{{item.doctor}}</td>
                                <td>{{item.result}}</td>
...
export default {
  ...
  data() {
    return {
      ...
      diagnoses: [{ id: 1, diagnosis: "", type: "New" }], // Initial diagnosis row,
      ...
      examOptions: {
        ...
        Microbiology: [
          ...
          'Fungal Culture',
          'Viral Load Testing (HIV, Hepatitis)',
          'Mycobacterial Culture (for TB)',
          'Stool Culture',
          'PCR (Polymerase Chain Reaction) for Pathogen Detection',
          'Serology Testing (for various infections)',
          'Rapid Antigen Testing (for various viruses)',
        ],
        Pathology: [
        ...
        Specialized: [
          'Urine Drug Screen',
          'Blood Typing',
          'Crossmatch (for Transfusions)',
          'Thrombophilia Panel',
          'Infectious Disease Panels (e.g., respiratory pathogens)',
        ]},
...
// return the array of pages
    ...
    addExam() {
      this.exams.push({ type: '', exam: '' });
    },
    removeExam(index) {
      this.exams.splice(index, 1);
    },
    filterExams(exam) {
      exam.exam = ''; // Reset selected exam when type changes
    },
    addExamination() {
      this.loading = true
      const data = new FormData();
      data.append("function", "addExamination");
      data.append("exams", JSON.stringify(this.exams));
    ...
    getExamination() {
      const data = new FormData();
      data.append("function", "getExamination");
      data.append("diagnoses", JSON.stringify(this.diagnoses));
      data.append("visitCode", sessionStorage.getItem("visitCode"));
      data.append("idNo", sessionStorage.getItem("idNo"));
      execute(data, patient)
          .then((res) => {
            ...
            if (res.data.success) {
              ...
              if (res.data && res.data.data && res.data.data.exams) {
                this.exams = res.data.data.exams
              }

              if (res.data && res.data.data && res.data.data.examinations) {
                this.examinations = res.data.data.examinations
              }
    ...
    addToBill(revenueStreamItem) {
      ...
      for (const item of this.invoiceItem) {
        ...
        this.billItem.push({
          ...
          subCountyID:sessionStorage.getItem("subCountyID"),
          subCountyName:sessionStorage.getItem("subCountyName"),
          wardID:sessionStorage.getItem("wardID"),
          wardName: sessionStorage.getItem("wardName"),
          ...
          description: this.patientsDetails.patient.names,
          patientID: this.patientsDetails.patient.idNo,
        });
      }
